...
const BentoMessage = () => {
    return (
        <div className="relative mx-auto flex h-48  w-full items-center justify-center  overflow-hidden rounded-xl">
            <div className="absolute  left-1/2 top-1/2 z-0 h-8 w-[calc((100%-48px)/4*3)]  origin-top-left animate-spin-slow ease-linear bg-gradient-to-t  from-white" />
            <div
                style={{
                    background:
                        'radial-gradient(circle at 0px 0px, hsl(var(--primary))  0%, hsl(var(--accent)) 100%)',
                }}
                className="z-10 flex h-48 mx-[4px] w-full  flex-col gap-2 rounded-lg"
            >
                <h1 className="ml-4 mt-4 text-xl lg:text-2xl text-primary-foreground ">
                    Messages <span className=' animate-pulse'>4</span>{' '}
                </h1>
                <AvatarGroup avatars={messageAvatars} className="ml-auto h-8" />
                <h1 className="ml-4 text-xl lg:text-2xl  text-primary-foreground">
